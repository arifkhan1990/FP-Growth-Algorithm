...
map<string , int> firstItem_for_tree;
map<string , int> Item_for_tree;
map<string , int> Item_for_tree1;
map<pair<string,string> , int > Full_FPTree_DaigramData;
...
void full_patteren_1(){
    map<string,int>::iterator pp = firstItem_for_tree.begin();
        if(firstItem_for_tree.size() == 1){
        cout << "\t\t\t\t\t  /\n";
        cout << "\t\t\t\t\t /\n";
        cout << "\t\t\t\t\t/\n";
        cout << "\t\t\t\t    " << pp->first << " : " << pp->second << " \n";
    }else if(firstItem_for_tree.size() == 2){
        cout << "\t\t\t\t\t    / \\ \n";
        cout << "\t\t\t\t\t   /   \\ \n";
        cout << "\t\t\t\t\t  /     \\ \n";
        cout << "\t\t\t\t      " << pp->first << " : " << pp->second;
    ...
    }else if(firstItem_for_tree.size() == 3){
        cout << "\t\t\t\t\t     /  |  \\ \n";
        cout << "\t\t\t\t\t    /   |   \\ \n";
        cout << "\t\t\t\t\t   /    |    \\ \n";
        cout << "\t\t\t\t      " << pp->first << " : " << pp->second;
...
void BodyPatteren(){
        map<string,int>::iterator pp = Item_for_tree.begin();
        map<string,int>:: iterator pp1 = Item_for_tree1.begin();
       // cout << Item_for_tree.size() << " " << Item_for_tree1.size() << endl;
        if(Item_for_tree.size() == 1 and Item_for_tree1.empty()){
        cout << "\t\t\t\t      /\n";
        cout << "\t\t\t\t     /\n";
        cout << "\t\t\t\t    /\n";
        cout << "\t\t\t        " << pp->first << " : " << pp->second << " \n";
    }else if(Item_for_tree.size() == 1 and Item_for_tree1.size() == 1){
        cout << "\t\t\t\t      /\t\\ \n";
        cout << "\t\t\t\t     /\t\\ \n";
        cout << "\t\t\t\t    /\t\\ \n";
        cout << "\t\t\t        " << pp->first << " : " << pp->second <<"\t " << pp1->first << " : " << pp1->second << " \n";
    }else if(Item_for_tree.size() == 2 and Item_for_tree1.size() == 2){
        cout << "\t\t\t\t         / \\\t     / \\ \n";
        cout << "\t\t\t\t        /   \\\t   /   \\ \n";
        cout << "\t\t\t\t       /     \\\t /     \\ \n";
        cout << "\t\t\t           " << pp->first << " : " << pp->second ;
    ...
    }else if(Item_for_tree.size() == 3 and Item_for_tree1.size() == 3){
        cout << "\t\t\t\t       /|\\ \t   /|\\ \n";
        cout << "\t\t\t\t      / | \\ \t  / | \\ \n";
        cout << "\t\t\t\t     /  |  \\ \t /  |  \\ \n";
        cout << "\t\t\t\t        |           |      \n";
...
void TreeBody_Patteren(){
    map<pair<string,string> , int > :: iterator pp = Full_FPTree_DaigramData.begin();
    map<string,int>::iterator pp1 = firstItem_for_tree.begin();
    int i = 0;
    while(pp1 != firstItem_for_tree.end() and i <= minimum_support){
           //cout << ++i << endl;
           i++;
            string sd = pp1->first;
            //pp1++;
    for(pp = Full_FPTree_DaigramData.begin();pp != Full_FPTree_DaigramData.end(); pp++){
            //cout<< pp->first.first << " " << pp->first.second << " " << pp->second << endl;
            if(sd == pp->first.second){
              Item_for_tree[pp->first.first] = pp->second;
             // cout << pp->first.first << endl;
              if(firstItem_for_tree.count(pp->first.first) == 0)
                 firstItem_for_tree[pp->first.first] = 0;
            }
            //cout << Item_for_tree.size() << endl;
    }
    pp1++;
    sd = pp1->first;
    for(pp = Full_FPTree_DaigramData.begin();pp != Full_FPTree_DaigramData.end(); pp++){
            if(sd == pp->first.second){
              Item_for_tree1[pp->first.first] = pp->second;
              //cout << pp->first.first << endl;
              if(firstItem_for_tree.count(pp->first.first) == 0)
                 firstItem_for_tree[pp->first.first] = 0;
            }
   ...
   //take uesr input
    ...
    for(int i = 0, j2 ; i < t_transaction ; i++){
        ...
        for(;it != order_set.end(); it++,j2++){
             cout <<  it->first << " ";
             string info = it->first;
             if(!info.empty() and b) {
                    firstItem_for_tree[it->first]++;
                    b = false;
             }
    ...
    for(; it != order.end(); it++){
      for(int i1 = 0; i1 < t_transaction ; i1++){
        for(int j1 = 1; database[i1][j1] != "*" ; j1++){
                if(database[i1][j1] == it->first){
                        string info = database[i1][j1-1];
                        if(!info.empty())
                        Full_FPTree_DaigramData[make_pair(it->first,database[i1][j1-1])]++;
                        break;
                }
        }
    }
    ...
    for(vector<pair<string,int> >::iterator ip = sort_low_value.begin(); ip != sort_low_value.end() - 1 ; ip++){
        ...
        // int k2 = 1 ;
          if(t_temporaray.count(ip->first) != 0)
              {
                  string Cpb = "";
                  for(vector<string>::iterator h3 = temporaryData.begin(); h3 !=  temporaryData.end()   ; h3++){
